  ...
  extends ComplexItem
{
  ...
  {
    WhileItem wi = new WhileItem(!this.b, this.w);
    if (this.istp.conditionslot.content != null)
    {
      wi.istp.conditionslot.add(this.istp.conditionslot.content.clone());
      wi.istp.conditionslot.content = this.istp.conditionslot.content.clone();
    }
    return wi;
  }
  
  public WhileItem(boolean preview, de.pylamo.spellmaker.gui.Window w)
  {
    super(w);
    setLayout(null);
    this.b = (!preview);
    ...
    if (preview)
    {
      this.istp = new ConditionStartPanel("while", true, w);
      createDragSource();
    }
    else
    {
      this.istp = new ConditionStartPanel("while", false, w);
  ...
  public void revalidate()
  {
    ...
    if (this.istp == null) {
      this.istp = new ConditionStartPanel("while", true, this.w);
    }
    if (this.eip == null) {
      this.eip = new EndPanel("endwhile");
    }
    ...
    implements DragGestureListener, DragSourceMotionListener
  {
    ...
    public void dragGestureRecognized(DragGestureEvent dge)
    {
      ...
      BufferedImage bi = new BufferedImage(WhileItem.this.getWidth(), WhileItem.this.getHeight(), 2);
      WhileItem.this.paint(bi.getGraphics());
      Cursor cursor = null;
...
/* Location:              C:\Users\Jens\Desktop\Spigot Server\plugins\Ancient_v1.1.0 (1).jar!\de\pylamo\spellmaker\gui\SpellItems\Condition\WhileItem.class
