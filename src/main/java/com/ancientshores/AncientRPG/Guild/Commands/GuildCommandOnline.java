    ...
    public static void processOnline(CommandSender sender) {
        ...
        if (guild != null) {
            ...
            for (String playersg : guildplayers) {
                if (AncientRPG.plugin.getServer().getPlayer(playersg) != null && AncientRPG.plugin.getServer().getPlayer(playersg).isOnline()) {
                    mPlayer.sendMessage(AncientRPGGuildRanks.getChatColorByRank(guild.gMember.get(playersg)) + playersg);
                }
            }
        } else {
