...
public class GuildCommandGrant {
    public static void processGrant(CommandSender sender, String[] args) {
        ...
        if (args.length == 3) {
            ...
            if (mGuild != null) {
                Player grantedplayer = AncientRPG.plugin.getServer().getPlayer(args[1]);
                if (grantedplayer != null && grantedplayer.isOnline() && mGuild.gMember.containsKey(grantedplayer.getName())) {
                        ...
                        switch (mGuild.gMember.get(mPlayer.getName())) {
                            case LEADER:
                                if (newrank == AncientRPGGuildRanks.LEADER && grantedplayer != mPlayer) {
                                    ...
                                    mGuild.gMember.put(grantedplayer.getName(), AncientRPGGuildRanks.LEADER);
                                    mGuild.gMember.put(mPlayer.getName(), AncientRPGGuildRanks.CO_LEADER);
                                    ...
                                    mGuild.gMember.put(grantedplayer.getName(), newrank);
