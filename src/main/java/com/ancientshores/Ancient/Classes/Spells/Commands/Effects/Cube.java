...
public class Cube extends ICommand {
	@CommandDescription(description = "<html>Creates a cube at the given location</html>",
			argnames = {"location", "particlename", "particles", "edge length", "enable rotation", "angular velocity x", "angular velocity y", "angular velocity z", "outline only", "period", "iterations"}, name = "Cube", parameters = {ParameterType.Location, ParameterType.String, ParameterType.Number, ParameterType.Number, ParameterType.Boolean, ParameterType.Number, ParameterType.Number, ParameterType.Number, ParameterType.Boolean, ParameterType.Number, ParameterType.Number})
	public Cube() {
	...
	public boolean playCommand(final EffectArgs ca) {
		if (ca.getParams().size() == 11
			...
			boolean enableRotation = (Boolean) ca.getParams().get(4);
			...
			boolean outlineOnly = (Boolean) ca.getParams().get(8);
