...
public class Shield extends ICommand {
	@CommandDescription(description = "<html>Creates a particle shield at the given location</html>",
			argnames = {"location", "particlename", "radius", "particles", "full sphere(true) or half sphere(false)", "period", "iterations"}, name = "Shield", parameters = {ParameterType.Location, ParameterType.String, ParameterType.Number, ParameterType.Number, ParameterType.Boolean, ParameterType.Number, ParameterType.Number})
	public Shield() {
	...
	public boolean playCommand(final EffectArgs ca) {
		if (ca.getParams().size() == 7
				...
				&& ca.getParams().get(4) instanceof Boolean
			...
			boolean isSphere = (Boolean) ca.getParams().get(4);
