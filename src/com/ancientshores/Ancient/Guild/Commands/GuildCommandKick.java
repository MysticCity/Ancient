                ...
                if (guild.gMember.containsKey(Bukkit.getOfflinePlayer(args[1]).getUniqueId())) {
                    if (rank == AncientGuildRanks.LEADER) {
                        if (!(guild.gMember.get(Bukkit.getOfflinePlayer(args[1]).getUniqueId()) == AncientGuildRanks.LEADER)) {
                            ...
                            mPlayer.sendMessage(GuildBrand.getDefaultGuildBrand() + ChatColor.RED + "You cannot kick yourself as a Leader.");
                        }
                        ...
                        if (!(guild.gMember.get(Bukkit.getOfflinePlayer(args[1]).getUniqueId()) == AncientGuildRanks.LEADER) && !(guild.gMember.get(Bukkit.getOfflinePlayer(args[1]).getUniqueId()) == AncientGuildRanks.CO_LEADER)) {
                            ...
                            mPlayer.sendMessage(GuildBrand.getDefaultGuildBrand() + ChatColor.RED + "You are not allowed to kick this player.");
                        }
                    } else {
                        mPlayer.sendMessage(GuildBrand.getDefaultGuildBrand() + ChatColor.RED + "You don't have the required rank to kick a Player.");
                    }
                } else {
                    mPlayer.sendMessage(GuildBrand.getDefaultGuildBrand() + ChatColor.RED + "This player isn't in your guild or does not exist, please use the exact name.");
                }
            } else {
                mPlayer.sendMessage(GuildBrand.getDefaultGuildBrand() + ChatColor.RED + "You aren't in a guild.");
            }
        } else {
            mPlayer.sendMessage(GuildBrand.getDefaultGuildBrand() + ChatColor.RED + "Correct usage /guild kick <name>");
        }
    }
}