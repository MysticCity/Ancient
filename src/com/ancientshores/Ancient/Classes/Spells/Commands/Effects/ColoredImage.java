...
public class ColoredImage extends ICommand {
	@CommandDescription(description = "<html>Shows a colored image at the given location</html>",
			argnames = {"location", "particlename", "file name", "show every x pixel", "show every y pixel", "size", "enable rotation", "rotation axis", "angular velocity x", "angular velocity y", "angular velocity z", "period", "iterations"}, name = "ColoredImage", parameters = {ParameterType.Location, ParameterType.String, ParameterType.String, ParameterType.Number, ParameterType.Number, ParameterType.Number, ParameterType.Boolean, ParameterType.String, ParameterType.Number, ParameterType.Number, ParameterType.Number, ParameterType.Number, ParameterType.Number})
	public ColoredImage() {
	...
	public boolean playCommand(final EffectArgs ca) {
		if (ca.getParams().size() == 13
				...
				&& ca.getParams().get(7) instanceof String
				&& ca.getParams().get(6) instanceof Boolean
			...
			boolean enableRotation = (Boolean) ca.getParams().get(6);
