...
public class Tornado extends ICommand {
	@CommandDescription(description = "<html>Creates a tornado at the given location</html>",
			argnames = {"location", "tornado particlename", "cloud particlename", "cloud size", "y offset", "height of tornado", "max tornado radius", "show cloud", "show tornado", "distance between rows", "period", "iterations"}, name = "Tornado", parameters = {ParameterType.Location, ParameterType.String, ParameterType.String, ParameterType.Number, ParameterType.Number, ParameterType.Number, ParameterType.Number, ParameterType.Boolean, ParameterType.Boolean, ParameterType.Number, ParameterType.Number, ParameterType.Number})
	public Tornado() {
	...
	public boolean playCommand(final EffectArgs ca) {
		if (ca.getParams().size() == 12
			...
			boolean showCloud = (Boolean) ca.getParams().get(7);
			boolean showTornado = (Boolean) ca.getParams().get(8);
