...
public class AncientSpellListener implements Listener {
	...
	public static void clearAll() {
		AncientSpellListener.damageEventSpells.clear();
		AncientSpellListener.damageEventBuffs.clear();
		AncientSpellListener.damageByEntityEventBuffs.clear();
		AncientSpellListener.damageByEntityEventSpells.clear();
		AncientSpellListener.joinEventBuffs.clear();
		AncientSpellListener.joinEventSpells.clear();
		AncientSpellListener.interactEventBuffs.clear();
		AncientSpellListener.interactEventSpells.clear();
		AncientSpellListener.attackEventBuffs.clear();
		AncientSpellListener.attackEventSpells.clear();
		AncientSpellListener.ChangeBlockEventBuffs.clear();
		AncientSpellListener.ChangeBlockEventSpells.clear();
		AncientSpellListener.chatEventBuffs.clear();
		AncientSpellListener.chatEventSpells.clear();
		AncientSpellListener.regenEventBuffs.clear();
		AncientSpellListener.regenEventSpells.clear();
		AncientSpellListener.moveEventBuffs.clear();
		AncientSpellListener.moveEventSpells.clear();
		AncientSpellListener.playerDeathBuffs.clear();
		AncientSpellListener.playerDeathSpells.clear();
		AncientSpellListener.LevelupEventBuffs.clear();
		AncientSpellListener.LevelupEventSpells.clear();
		AncientSpellListener.ProjectileHitEventBuffs.clear();
		AncientSpellListener.ProjectileHitEventSpells.clear();
		AncientSpellListener.classChangeEventBuffs.clear();
		AncientSpellListener.classChangeEventSpells.clear();
		ignoredEvents.clear();
	}
	...
	public void onPlayerKill(final EntityDamageByEntityEvent event) {
		...
		if (damager instanceof Player && !AncientExperience.alreadyDead.contains(event.getEntity().getUniqueId())) {
			if (event.getEntity() instanceof LivingEntity && ((LivingEntity) event.getEntity()).getHealth() - event.getDamage() <= 0) {
				...
				/*
				 * Bukkit.getScheduler().scheduleSyncDelayedTask(Ancient.plugin
	...
	public void onPlayerChat(final AsyncPlayerChatEvent event) {
		...
		Bukkit.getScheduler().scheduleSyncDelayedTask(AncientSpellListener.plugin, new Runnable() {
	...
	public void onEntityDeath(final EntityDeathEvent event) {
		...
		Player mPlayer = Bukkit.getServer().getPlayer(AncientEntityListener.scheduledXpList.get(entityUUID));
		PlayerData pd = PlayerData.getPlayerData(mPlayer.getUniqueId());
		HashMap<Spell, UUID[]> spells = new HashMap<Spell, UUID[]>();
		for (Spell p : killEntityEventSpells) {
			if (AncientClass.spellAvailable(p, pd)) {
